...
function onLoad(): CydranJson {
	...
	} else if (existsSync(homedir() + '/.vscode/extensions/tannersmith.cydran-intellisense-0.0.15/out/cydran.json')) {
		console.log('prod json loaded');
		return JSON.parse(readFileSync(homedir() + '/.vscode/extensions/tannersmith.cydran-intellisense-0.0.15/out/cydran.json', 'utf-8'));
	} else {
	...
	const htmlCommands = vscode.languages.registerCompletionItemProvider(
		'html',
		{
			provideCompletionItems(document: vscode.TextDocument, position: vscode.Position, token: vscode.CancellationToken, context: vscode.CompletionContext) {
				...
				// console.log(document.lineAt(position).text.substring(0, position.character));
